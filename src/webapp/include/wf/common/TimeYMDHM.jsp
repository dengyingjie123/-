...
String strHour    =    new String();    //保存小时数
String strMinute  =    new String();    //保存分钟数
String strAmPm    =    new String();    //获得上午下午
...
if (strTime != null && !strTime.equals("")) {
  ...
  strHour = String.valueOf(cal.get(Calendar.HOUR));            //获得初始化小时数
  strMinute = String.valueOf(cal.get(Calendar.MINUTE));        //获得初始化分钟数
  strAmPm = String.valueOf(cal.get(Calendar.AM_PM));           //获得初始化上午下午
  //判断如果时间是下午，将小时变为24小时制显示
  if (strAmPm.equals(String.valueOf(Calendar.PM))) {
    strHour = String.valueOf(Double.parseDouble(strHour) + 12);
  }
}
%>
<table width="100%"  border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td><div align="center">
        <select name="<%=id%>_hour_Time" id="<%=id%>_hour_Time" onChange="<%=id%>Change()">
        </select>
    </div></td>
    <td><div align="center">时</div></td>
    <td><div align="center">
        <select name="<%=id%>_minute_Time" id="<%=id%>_minute_Time" onChange="<%=id%>Change()">
        </select>
    </div></td>
...
function <%=id%>Change() {
  ...
  document.all.<%=id%>.value = document.all.<%=id%>_nian_Time.value + "-" +
                               ...
                               document.all.<%=id%>_hour_Time.value + ":" +
                               document.all.<%=id%>_minute_Time.value;
}
function <%=id%>initTime() {
  ...
  // 初始化小时 Hour
  var thisHour = <%=strHour.equals("")?"now.getHours()":strHour%>;
  var <%=id%>_hour_Time = document.all.<%=id%>_hour_Time;
  for (i = 0; i < 24; i++) {
    var hours = document.createElement("option");
    hours.text = i;
    hours.value = i;
    <%=id%>_hour_Time.add(hours);
  }
  <%=id%>_hour_Time.options[thisHour].selected = true;
  // 初始化分钟 Minute
  var thisMinute = <%=strMinute.equals("")?"now.getMinutes()":strMinute%>;
  var <%=id%>_minute_Time = document.all.<%=id%>_minute_Time;
  for (i = 0; i < 60; i++) {
    var minutes = document.createElement("option");
    minutes.text = i;
    minutes.value = i;
    <%=id%>_minute_Time.add(minutes);
  }
  <%=id%>_minute_Time.options[thisMinute].selected = true;
}
<%=id%>initTime();  //初始化时间
<%=id%>Change();  //更新ID隐藏域中的数据

//启用或禁用表单
function <%=id%>Disable(boolTarget) {
  document.all.<%=id%>_nian_Time.disabled = boolTarget;
  document.all.<%=id%>_month_Time.disabled = boolTarget;
  document.all.<%=id%>_day_Time.disabled = boolTarget;
  document.all.<%=id%>_hour_Time.disabled = boolTarget;
  document.all.<%=id%>_minute_Time.disabled = boolTarget;
}
</script>
