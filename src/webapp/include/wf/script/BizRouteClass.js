 ...
 *   using('../wf/script/BizRouteClass.js', function () {
                var bizRoute = new BizRouteClass(token, YWID,workflowID, routeListId, currentNodeId,tableId,windowId);
                bizRoute.initModule();
                process.afterClick();
            });
 *
 ...
 * @param token
 * @param YWID 业务编号ID
 * @param WorkflowID 当前工作流编号
 * @param RouteListID 路由节点
 * @param CurrentNode  当前节点编号
 * @param TableId 列表ID
 * @param TableId2 列表ID2
 * @param TableId3 列表ID3
 * @param windowID 需要关闭的窗口ID
 * @returns {{initModule: initModule}}
 *YWID, WorkflowID, RouteListID, CurrentNode = YWData
 */
var BizRouteClass = function (token, YWData, TableId, TableId2, TableId3, windowID, newWorkflow) {

    function initAll() {
        //初始获取业务数据的方法
        onClickBizRoute(YWData, newWorkflow);
    }
     ...
     *
     * 用法
     * onClickBizRoute(YWID, WorkflowID, RouteListID, CurrentNode);
     *
     * 修改：周海鸿
     * 内容：完成详细注释
     * 时间：2015年 6月17日 09:24:00
     *
     *
     * @param buttonId 触发事件的按钮编号
     * @param YWID 业务编号ID
     * @param WorkflowID 当前工作流编号
     * @param RouteListID  路由节点
     * @param CurrentNode 当前节点编号
     */
    function onClickBizRoute(YWData, newWorkflow) {
        ...
        fw.post(url, null, function (data) {
            ...
            //调用业务数据控制窗口
            initWindowBizRouteWindow(data, newWorkflow);
     ...
     * @param date 表单加载数据
     */
    function initWindowBizRouteWindow(data, newWorkflow) {
        var bizRouteJsp = "";
        if (newWorkflow != null && newWorkflow == 1) {
            bizRouteJsp = "/include/wf/bizRoute/bizRouteNew_Save.jsp";
        } else {
            bizRouteJsp = "/include/wf/bizRoute/bizRoute_Save.jsp";
        }

        var windowId = "bizRouteWindow" + token;

        //组装业务控制页面的URL
        var param = "?YWID=" + data["YWID"] + "&WorkflowID=" + data["WorkflowID"] + "&UserID=" + data["Participant"] + "&RouteListID=" + data['RouteListID'] + "&CurrNodeID=" + data['CurrentNode'] + "&token=" + token;
        var url = WEB_ROOT + bizRouteJsp + param;

        //弹出业务填写页面
        fw.window(windowId, '业务流转', 600, 360, url, function () {


            if (YWData["currentStatus"] == 5) {
                $("#btnbizRouteSubmit" + token).remove();
            }
            ...
            checkFromEditable();
            //业务转发提交事件
            onClickBizRouteSubmit(statusobj);

            //业务结束事件
            onClickBinRouteOver(statusobj);
            //业务回退事件
            onClickBinRouteReject(statusobj);
            //业务中止事件
            onClickBinRouteCancel(statusobj);
        });
    }
     ...
     *
     */
    function onClickBizRouteSubmit(statusobj) {
        //设置提交按钮
        var buttonId = "btnbizRouteSubmit" + token;
     ...
     *
     */
    function onClickBinRouteOver(statusobj) {
            //设置结束按钮id
            var buttonId = "btnbizRouteOver" + token;
     ...
     * @param statusobj
     */
    function onClickBinRouteReject(statusobj) {
        //设置结束按钮id
        var buttonId = "btnbizRouteReject" + token;
     ...
     */
    function onClickBinRouteCancel(statusobj) {
            //设置结束按钮id
            var buttonId = "btnbizRouteCancel" + token;
