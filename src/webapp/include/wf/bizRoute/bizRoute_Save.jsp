    ...
    <form id="formBizRoute<%=token %>" name="formBizRoute" action="" method="post">
        <div region="center" border="false" style="padding:10px;background:#fff;border:0px solid #ccc;">
            ...
            <table width="500" border="0   " cellspacing="5" cellpadding="0">
                ...
                <tr style="display: <%=ClientApplications.getFieldStatus(intWorkflowID, intCurrNodeID, "chargeLeaderContent")  == 2 ? "none;" : ""%>">
                    <td align="right"
                        style="<%=ClientApplications.getFieldCSS(intWorkflowID, intCurrNodeID, "chargeLeaderContent")%>">
                        分管领导意见
                    </td>
                    <td colspan="3"><textarea type="text" id="chargeLeaderContent<%=token %>"
                                              name="bizRoute.chargeLeaderContent"
                                              style="resize:none;width:350px;height: 40px"
                                              wf="<%=ClientApplications.getFieldStatus(intWorkflowID, intCurrNodeID, "chargeLeaderContent")  == 0 ? "0" : "1"%>"/>
                    </td>
                </tr>
                <tr style="display: <%=ClientApplications.getFieldStatus(intWorkflowID, intCurrNodeID, "chargeLeaderContent")  ==  2 ? "none;" : ""%>">
                    <td align="right"
                    ...
                    <td><input type="text" id="chargeLeaderName<%=token %>" name="bizRoute.chargeLeaderName"
                               style="width:140px"/></td>
                    <td align="right"
                        style="<%=ClientApplications.getFieldCSS(intWorkflowID, intCurrNodeID, "chargeLeaderContent")%>">
                        时间
                    </td>
                    <td><input type="text" id="chargeLeaderTime<%=token %>" name="bizRoute.chargeLeaderTime"
                               style="width:140px"/></td>
        ...
        <div region="south" border="false" style="text-align:right;padding:5px;background:#F4F4F4">
            ...
            <div style="text-align:left;color: red;">说明：以前需要选择节点再提交,新修改为点同意自动到下一节点,下一节点位置可以在列表最后一列查看。另外最后一流程节点直接点击【完成】</div>
            <%
                Node node = Node.searchNodeObject(intWorkflowID, intCurrNodeID);
                if (node.getType() == 2) {%>
            <a id="btnbizRouteSubmit<%=token %>" class="easyui-linkbutton" iconCls="icon-ok"
               href="javascript:void(0)">同意</a>
            <a id="btnbizRouteOver<%=token %>" class="easyui-linkbutton" iconCls="icon-ok"
               href="javascript:void(0)">完成</a>
            <a id="btnbizRouteReject<%=token %>" class="easyui-linkbutton" iconCls="icon-ok"
               href="javascript:void(0)">回退</a>
            <a id="btnbizRouteCancel<%=token %>" class="easyui-linkbutton" iconCls="icon-ok"
               href="javascript:void(0)">中止</a>
            <%
            } else {
            %>
            <a id="btnbizRouteSubmit<%=token %>" class="easyui-linkbutton" iconCls="icon-ok"
            ...
            <a class="easyui-linkbutton" iconCls="icon-cancel" href="javascript:void(0)"
               onClick="fwCloseWindow('bizRouteWindow<%=token%>')">取消</a>
        </div>
    </form>
</div>
</body>
