 ...
 *
 *  用法
 *    using(SCRIPTS_ROOT + '/oa/finace/FinanceExpenseClass.js', function () {
            var financeExpenseClass = new FinanceExpenseClass(token);
            financeExpenseClass.initModule();
        });
 *
 *  修改：周海鸿
 *  内容：完成详细注释
...
var FinanceExpenseClass = function (token) {
    var expenseId;
    ///  初始化部分 开始  /////////////////////////////////////////////////////////////////
...
//        初始化部门与下了列表
    ...
    function initFinanceExpenseTable() {
        ...
        var url = WEB_ROOT + "/oa/finance/FinanceExpense_list.action";
        $('#' + strTableId).datagrid({
//            title: '费用报销',
            url: url,
            queryParams: {
    ...
    function initWaitFinanceExpenseTable() {
        ...
        var url = WEB_ROOT + "/oa/finance/FinanceExpense_waitlist.action";
        $('#' + strTableId).datagrid({
//            title: '费用报销',
            url: url,
            queryParams: {
    ...
    function initParticipantFinanceExpenseTable() {
        ...
        var url = WEB_ROOT + "/oa/finance/FinanceExpense_participantlist.action";
        $('#' + strTableId).datagrid({
//            title: '费用报销',
            url: url,
            queryParams: {
    ...
    function initWindowFinanceExpenseWindow(data, obj, windostatus) {
        ...
        if (windostatus == "1") {
            ...
            window = "655";
            height = "600";
        } else if (windostatus == "3" || windostatus == "look") {
            window = "655";
            height = "580";
        }

        var url = WEB_ROOT + "/modules/oa/finance/FinanceExpense_Save.jsp?&token=" + token;
        ...
        fw.window(windowId, '申请', window, height, url, function () {
            ...
            if (windostatus == "3") {
                ...
                fw.onClickBizSubmit(token, "btnFinanceExpenseSubmit_Start" + token,
                    data , "FinanceExpenseTable" + token, 'WaitFinanceExpenseTable' + token, 'ParticipantFinanceExpenseTable' + token, windowId);
                temp = true;
            } else {
                //添加数据提交事件
...
////    }
    ...
    function onClickFinanceExpenseSubmit2() {
        ...
        fw.bindOnClick(buttonId, function (process) {
            ...
            var formId = "formFinanceExpense" + token;
            var url = WEB_ROOT + "/oa/finance/FinanceExpense_insertOrUpdate.action";
