...
var FinanceBizTripExpenseWFA4DetailNewClass = function (token)
{
...
//            title: '差旅报销',
            ...
            columns: [
                [
                    ...
                    { field: 'generalManagerContent', title: '副总裁审核内容',
                        formatter: function (value, row, index) {
...
//            title: '差旅报销',
    ...
    function onClickFinanceBizTripExpenseWFAPrint(buttonId,tableid) {
        ...
        fw.bindOnClick(buttonId, function (process) {
            fw.datagridGetSelected(tableid, function (selected) {
                ...
                var url =WEB_ROOT+"/modules/oa/modelsFiles/FinanceBizTripExpenseltemModelsNew.jsp?id="+selected.id+"&token=" + token;
                window.open(url);
    ...
    // HOPEWEALTH-1360
    function AAAinitFinanceBizTripExpenseWorkflowPanel(data) {
        // 组装URL
        ...
        url += "&token=" + token;
        $('#panelTripExpenseWorkflow' + token).panel({
            href:url,
            onLoad:function() {
                //直接引用工作流的js代码，将按钮与功能绑定，并实现编辑控制
                using('../wf/script/BizRouteNewClass.js', function () {
                    // 组装参数，传入token, data, 三个表的ID, 窗口ID和四个按钮ID
                    var table1 = "financeBizTripExpenseWFATable" + token;// 我的申请列表
                    var table2 = 'WaiitfinanceBizTripExpenseWFATable' + token;// 等待我审核列表
                    var table3 = 'ParticipantfinanceBizTripExpenseWFATable' + token;// 已完成列表
                    var windowId = 'financeBizTripExpenseWFAWindow' + token;// 窗口ID
                    var btn1 = "btnFinanceBizTripExpenseWFABizRouteSubmit";// 同意按钮
                    var btn2 = "btnFinanceBizTripExpenseWFABizRouteOver";// 完成按钮
                    var btn3 = "btnFinanceBizTripExpenseWFABizRouteReject";// 回退按钮
                    var btn4 = "btnFinanceBizTripExpenseWFABizRouteCancel";// 中止按钮
    ...
    function initFinanceBizTripExpenseWFAWindow(data, temp, windowStatus) {
        ...
        //设置窗口数据请求URL
        var url = WEB_ROOT + '/modules/oa/expense/FinanceBizTripExpenseWFA4DetailNew_Save.jsp?token=' + token;
        //设置窗口ID
        ...
        fw.window(windowId, '差旅报销', width, height, url, function () {
            ...
            if (temp == "check") {
                ...
                $("#panelTripExpenseWorkflow" + token).remove();
                $("#btnFinanceBizTripExpenseWFABizRouteSubmit" + token).remove();
                $("#btnFinanceBizTripExpenseWFABizRouteOver" + token).remove();
                $("#btnFinanceBizTripExpenseWFABizRouteReject" + token).remove();
                $("#btnFinanceBizTripExpenseWFABizRouteCancel" + token).remove();
            }
            // 结束节点为8
            if (data["CurrentNode"] == "8" || data["CurrentNode"] == 8) {
                $("#btnFinanceBizTripExpenseWFABizRouteSubmit" + token).remove();// 移除同意按钮，保留完成按钮
            } else {
                $("#btnFinanceBizTripExpenseWFABizRouteOver" + token).remove();// 移除完成按钮，保留同意按钮
            }
            ...
            //窗口为业务审批(流程中)状态
                ...
                // HOPEWEALTH-1360
                AAAinitFinanceBizTripExpenseWorkflowPanel(data);
