...
var ProvincesSelectClass = function (token, obj) {

    //初始化方法
    function initAll() {
        //初始化按钮提交事件
        onClickProvincesSelectSubmit();
        select();
    }
...
* 选择按钮事件
*/
function onClickProvincesSelectSubmit() {
//确定按钮
var buttonId = "btnProvincesSubmit" + token;
fw.bindOnClick(buttonId, function (process) {;

    if(alert($('#Province'+token).val()) == 0){
        alert("请选择省份");
        return;
    }
    //獲取省份
    var ProvinceName=$('#Province'+token+" option:selected").text()
    var cityName=$('#City'+token+" option:selected").text();
    //关闭窗口
    fw.windowClose('ProvincesSelectWindow'+token);
    //回调函数
    obj.loadProvincesInfo(ProvinceName,cityName);
     ...
     */
    function select() {
        var SelectBtn = "Province" + token;
        $('#Province' + token).bind('change', function ()
        {
            var x = [35];
            x[0] = "-请选择-";
            ...
            //获取数据数组
            var citys = x[document.getElementById("Province"+token).value].split(",");
            //清空下拉列表
            $("#City"+token).empty();
            //将数组数据绑定到列表中
            for (var i = 0; i < citys.length; i++)
                document.getElementById("City"+token).options[i] = new Option(citys[i], citys[i]);
