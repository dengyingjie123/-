...
function initModule() {
    //initProductionInfoTitle();
    initProductionInfoContent();

    initProductionCompositions();
 ...
 * 初始化产品文章内容
 */
function initProductionInfoContent() {
    var productionId = fweb.getQueryString("productionId");
    var url = "/core/api/production/ProductionInfo_getProductionInfoContent";
    var params = {'productionId': productionId};

    fweb.post(url, params, function (data) {
        var rows = data['returnValue']['rows'];
        var e = '<div class="home-recommend-ct"><h3 class="tit"><i class="sjx-ico1"></i><i class="triangle-right"></i>{title}</h3><div class="pro-hights pd25">{contentShort}<span class="showall-ct"style="display: none;">{content}</span><span class="showall">【显示全部】</span></div></div>';
        var info = "";

        for (var i = 0; i < rows.length; i++) {
            // 将产品信息加入html内容
            var t = e.replace('{title}', rows[i]['title']);
            t = t.replace('{contentShort}', rows[i]['content'].substring(0, defaultLength));
            t = t.replace('{content}', rows[i]['content'].substring(defaultLength));
            info = info + t;
        }
        $(".pro-details").append($.parseHTML(info));

        $(function () {
            $(".showall").click(function () {
                $(this).siblings(".showall-ct").toggle();
            });
            $(".ct-open .tit").click(function () {
                $(this).siblings(".sh-ct").slideToggle();
            });
        });
...
function initProductionInfoTitle() {
    var productionId = fweb.getQueryString("productionId");
    var url = "/core/api/production/ProductionInfo_getProductionInfoTitle";
    var params = {'productionId': productionId};
    fweb.post(url, params, function (data) {
        var rows = data = data['returnValue']['rows'];
        var e = '<div class="home-recommend-ct ct-open"><h3 class="tit h10"><i class="sjx-ico1"></i><i class="triangle-right"></i>{title}</h3><input type="hidden" value="{value}" /><i class="icon-arrow"></i></div>';
        var info = "";
        var t = "";

        for (var i = 0; i < rows.length; i++) {
            t = e.replace('{title}', rows[i]['title']);
            t = t.replace('{value}', rows[i]['productionId'] + '|' + rows[i]['index']);
            info = info + t;
        }
        $(".pro-details").append($.parseHTML(info));
...
function initBaseInfo() {
    var productionId = fweb.getQueryString("productionId");
    var url = "/core/api/production/Production_loadProduction";
    var params = {'productionId': productionId};
    fweb.post(url, params, function (data) {
        var rows = data = data['returnValue']['rows'];
        ...
        for (var i = 0; i < rows.length; i++) {
            t = rows[i];
            info = '<li>产品名称：<span class="data"> ' + t['name'] +
                '</span></li><li>发行规模：<span class="data"> ' + getFormatAmount(t['size']) +
                '</span></li><li>产品期限：<span class="data"> ' + t['investTermView'] +
                '</span></li><li>付息方式：<span class="data"> ' + t['interestType'] + '</span></li>';
        }
        $("#productionBaseInfo").append($.parseHTML(info));

        $(function () {

            $(".hd-cfact").click(function () {
                $(".zbc-box").show();
                $(".share-type").show();
            });
...
function initProductionCompositions() {
    var productionId = fweb.getQueryString("productionId");

    var url = "/core/api/production/ProductionComposition_listCompositions";
    var params = {
    ...
    fweb.post(url, params, function (data) {
        ...
        for (var i = 0; i < rows.length; i++) {
            t = rows[i];
            amount = amount + '<p>' + t['name'] + '</p>';
            rate = rate + '<p>' + t['expectedYield'] + '%</p>';
        }

        $("#productionNames").append($.parseHTML(amount));
        $("#productionRate").append($.parseHTML(rate));
    }, null);
}

function getFormatAmount(m) {
    if (m < 10000) {
        return m + "万";
    }
    m = m / 10000;
...
function invest(minMoney, restInvestMoney) {
    // 提交对按钮进行控制
...
//                verifyMoney.innerHTML = "您的资金不足&nbsp;&nbsp;<a href='/core/w2/customer/gotorecharge' style='color:#6d30b3' >立即充值</a>";
//
//                $('#investSubmmitButtons').removeAttr('disabled');
//                $('#investSubmmitButtons').val("投资");
//                return false;
