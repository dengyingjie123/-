...
%>
<%
    String orderType = com.youngbook.common.utils.HttpUtils.getParameter(request, "orderType");
    String customerId = com.youngbook.common.utils.HttpUtils.getParameter(request, "customerId");
...
<head>
    ...
    <script>
        ...
        app.controller('myCtrl', function($scope, $http){
            var url = "<%=Config.getWebRoot()%>/modern/c/production/Order_getOrders4Modern.action?orderType=<%=orderType%>&customerId=<%=customerId%>";
            $http.post(url, "").success(function(response){
                $scope.orders = response.returnValue;
            });

            $scope.showOrderDetails = function(orderId, productionId) {
                fm.goto('<%=Config.getModernCustomerManagementPages()%>/order/order_detail.jsp?orderId='+orderId+'&productionId='+productionId);
            }
        });
    </script>
</head>
        ...
        <div class="home-recommend-ct" ng-click="showOrderDetails(o.id, o.productionId)" ng-repeat="o in orders">
            ...
            <div class="pro-infos lbBox pd25-all">
                ...
                <div class="lineBlock pro-info-rt pro-proportion">
                    <p>预期年化收益率<span class="data"> {{o.expectedYield==0?"浮动":o.expectedYield+"%"}}</span></p>
