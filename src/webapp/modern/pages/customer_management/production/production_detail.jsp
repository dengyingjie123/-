...
<head>
    ...
    <script type="application/javascript">
        ...
        app.controller('myCtrl', function($scope, $http, $sce){
            $http.get('<%=Config.getWebRoot()%>/modern/c/production/Production_getProductionWVOById.action?productionId=<%=productionId%>').success(function(response){
    ...
    <section class="pro-details">
        ...
        <div class="home-recommend-ct">
            ...
            <div class="pro-hights pd25">
                ...
                <div class="fr-rt">
                    <button type="button" class="nowbuy" onclick="fm.goto('<%=Config.getModernCustomerManagementPages()%>/order/appointment.jsp?productionId=<%=productionId%>');">立即预约</button>
                    <button type="button" class="share-btn" onclick="fm.goto('<%=Config.getModernCustomerManagementPages()%>/production/production_detail_share.jsp?productionId=<%=productionId%>');">分享给朋友</button>
