...
pagePool.put('production-detail', {
    ...
    onPageInit:function (app, page) {
        ...
        fm.f7_bind('.btn-investment',function(e){
            ...
            fm.f7_post(myApp, urlCustomer, "", function(customerData){
                ...
                if (fm.checkIsNullObject(customerData['returnValue']['accountVOs']) || customerData['returnValue']['accountVOs'].length == 0) {
                    ...
                    url = WEB_ROOT + '/ph/market/order_save.jsp?productionId=' + productionId;
        ...
        $$('#btn-production-share').on('click', function (e) {
            ...
            var url = "/ph/share/share_production.jsp?productionId="+productionId+"&userId="+userId;
...
pagePool.put('customer-save', {
    ...
    onPageInit:function (app, page) {
        $$('.btn-customer-save').on('click', function(){
            ...
            if (fm.checkIsTextEmpty(formData['name'])) {
                fm.f7_alert(myApp, '请输入姓名');
                return;
            }

            if (fm.checkIsTextEmpty(formData['mobile'])) {
                fm.f7_alert(myApp, '请输入手机号');
                return;
            }
            ...
            var url = WEB_ROOT + '/dehesales/customerPersonal_register?';

            if (!fm.checkIsTextEmpty(formData['id'])) {
                // updateModern
                url = WEB_ROOT + '/dehesales/customerPersonal_update?';
            }
        ...
        $$('.btn-customer-remove').on('click',function(){
            var formData = myApp.formToJSON('#customer-save-form');
            // alert(JSON.stringify(formData));
            var data = {
            ...
            if (!fm.checkIsTextEmpty(formData['id'])) {
                var url = WEB_ROOT + '/dehesales/customerPersonal_remove?';
...
var homeListPage = {
    ...
    onPageInit:function (app, page) {
        $$('.btn-home-share').on('click', function (e) {
            ...
            var url = "/ph/share/share_article.jsp?userId="+userId+"&articleId="+articleId+"&title="+title;
            url = fm.urlEncode(url);
