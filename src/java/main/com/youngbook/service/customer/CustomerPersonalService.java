...
public class CustomerPersonalService extends BaseService {
     ...
     * @throws Exception
     */
    public Pager listCustomrs4DistributionToMe(CustomerPersonalVO personalVO, List<KVObject> conditions, int currentPage, int showRowCount, UserPO user) throws Exception {
        ...
        sbSQL.append("     c.id,");
        sbSQL.append("     c.`Name`,");
        sbSQL.append("     c.customerTypeId,");
        sbSQL.append("     c.PersonalNumber,");
        sbSQL.append("     c.LoginName,");
        sbSQL.append("     c.createTime,");
        sbSQL.append("     kvSex.v Sex,");
        sbSQL.append("     c.Mobile,");
        sbSQL.append("     c.Birthday,");
        sbSQL.append("     c.WorkAddress,");
        sbSQL.append("     c.HomeAddress,");
        sbSQL.append("     c.IdentityCardAddress,");
        sbSQL.append("     c.Phone,");
        sbSQL.append("     g.`Name` GroupName,");
        sbSQL.append("     u.`name` SaleManName,");
        sbSQL.append("     u.`id` SaleManId,");
        sbSQL.append("     cd.Status DistributionStatus,");
        sbSQL.append("     c.operateTime, ");
        sbSQL.append("     c.customerCatalogId, ");
        sbSQL.append("     c.customerChannelTypeId, ");
        sbSQL.append("     c.referralCode ");
        sbSQL.append(" FROM");
        sbSQL.append("     crm_customerpersonal c,");
        sbSQL.append("     crm_customerdistribution cd,");
        sbSQL.append("     system_user u,");
        sbSQL.append("     CRM_SaleMan sm,");
        sbSQL.append("     crm_saleman_salemangroup sg,");
        sbSQL.append("     CRM_SalemanGroup g,");
        sbSQL.append("     system_kv kvSex");
        sbSQL.append(" WHERE");
        sbSQL.append("     1 = 1");
        sbSQL.append(" AND c.state = 0");
        sbSQL.append(" AND cd.state = 0");
        sbSQL.append(" AND u.state = 0");
        sbSQL.append(" AND sm.state = 0");
        sbSQL.append(" AND g.state = 0");
        sbSQL.append(" AND cd.status = 1");
        sbSQL.append(" and cd.`Status`=1");
        sbSQL.append(" AND c.Sex = kvSex.k");
        sbSQL.append(" AND kvSex.GroupName = 'Sex'");
        sbSQL.append(" AND cd.CustomerId = c.id");
        sbSQL.append(" AND cd.SaleManId = sm.UserId");
        sbSQL.append(" AND sm.UserId = u.Id");
        sbSQL.append(" AND sg.saleManGroupId = g.Id");
        sbSQL.append(" AND sg.saleManId = u.Id");
        sbSQL.append(" AND cd.saleGroupId = g.id");
        sbSQL.append(" AND u.Id = '" + userId + "'");
        ...
        sbSQL.append("     '' SaleManName,");
        sbSQL.append("     '' SaleManId,");
        sbSQL.append("     3 DistributionStatus,");
        sbSQL.append("     c.operateTime, ");
        sbSQL.append("     c.customerCatalogId, ");
        sbSQL.append("     c.customerChannelTypeId, ");
        sbSQL.append("     c.referralCode ");
        ...
        sbSQL.append(" AND c.Sex = kvSex.k");
        sbSQL.append(" AND kvSex.GroupName = 'Sex'");
        sbSQL.append(" AND c.OperatorId = '" + userId + "'");
        sbSQL.append(" AND c.id NOT IN (");
        sbSQL.append("     SELECT");
        sbSQL.append("         CustomerId");
        sbSQL.append("     FROM");
        sbSQL.append("         crm_customerdistribution");
        sbSQL.append("     WHERE ");
    ...
    public CustomerPersonalPO updateModern(CustomerPersonalPO customer, String userId, Connection conn) throws Exception {
        ...
        CustomerPersonalPO tempCustomer = customerPersonalDao.loadByCustomerPersonalId(customer.getId(), conn);
