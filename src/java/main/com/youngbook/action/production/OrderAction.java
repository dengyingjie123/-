...
public class OrderAction extends BaseAction {
    ...
    public String list() throws Exception {
        ...
        // 销售负责人查看
            ...
            pager = orderService.showListSaleGroup(orderVO, payTimeStart, payTimeEnd, getLoginUser(), pager.getCurrentPage(), pager.getShowRowCount(), conn);
        }
        ...
        for (IJsonable data : datas) {
            ...
            // 设置订单状态名称
            String statusName = "未兑付";

            if (order.getPaymentPlanStatus() == 8) {
                statusName = "部分兑付";
            }

            if (order.getPaymentPlanStatus() == 5) {
                statusName = "已兑付";
            }
            ...
            if (order.getCustomerAttribute() != null) {
    ...
    public String exportExcel() throws Exception {
        ...
        OrderVO orderVO = HttpUtils.getInstanceFromRequest(getRequest(), "order", OrderVO.class);

        String search_PayTime_Start = getHttpRequestParameter("search_PayTime_Start");
        String search_PayTime_End = getHttpRequestParameter("search_PayTime_End");

        TimePO payTimePO = new TimePO();
        payTimePO.setStartTime(search_PayTime_Start);
        payTimePO.setStopTime(search_PayTime_End);

        Connection conn = getConnection();
        String fileName = Config.getSystemConfig("order_list_export_template_name");
        String title = Config.getSystemConfig("order_list_export_template_name");

        //转换输出格式防止乱码
        fileName = new String(fileName.getBytes("utf8"), "iso8859-1") + "_" + TimeUtils.getNowDateYYYYMMDD() + "_" + TimeUtils.getNowTimeHH24MMSS();
        ...
        try {
            FileInputStream fileInputStream = new FileInputStream(Config.getSystemConfig("order_list_export_template_file"));
            ...
            ExcelUtils.setCellValue("a1", title, sheet);

            List<OrderVO> listOrderVO = orderService.getListOrderVO(orderVO, payTimePO, null, conn);
            ...
            for (int i = 0; listOrderVO != null && i < listOrderVO.size(); i++) {
                ...
                ExcelUtils.setCellValue("a" + (offset + i), orderVO.getOrderNum(), sheet);
                ExcelUtils.setCellValue("b" + (offset + i), orderVO.getCustomerName(), sheet);
                ExcelUtils.setCellValue("c" + (offset + i), orderVO.getProductionName(), sheet);
                ExcelUtils.setCellValue("d" + (offset + i), orderVO.getMoney(), sheet);
                ExcelUtils.setCellValue("e" + (offset + i), orderVO.getAppointmentTime(), sheet);
                ExcelUtils.setCellValue("f" + (offset + i), orderVO.getPayTime(), sheet);
                ExcelUtils.setCellValue("g" + (offset + i), orderVO.getPaymentPlanLastTime(), sheet);
                ExcelUtils.setCellValue("h" + (offset + i), OrderStatus.getStatusName(orderVO.getStatus()), sheet);
                ExcelUtils.setCellValue("i" + (offset + i), orderVO.getSalesmanName(), sheet);
                ExcelUtils.setCellValue("j" + (offset + i), orderVO.getGroupName(), sheet);
                ExcelUtils.setCellValue("k" + (offset + i), orderVO.getProductionCompositionName(), sheet);
                ExcelUtils.setCellValue("l" + (offset + i), orderVO.getContractNo(), sheet);
                ExcelUtils.setCellValue("m" + (offset + i), orderVO.getFinanceMoneyConfirmUserName(), sheet);
                ExcelUtils.setCellValue("n" + (offset + i), orderVO.getFinanceMoneyConfirmTime(), sheet);
            }
    ...
    public String updateOrderProduction() throws Exception {
        ...
        orderService.updateOrderProduction(orderId, productionId, productionCompositionId, money,getLoginUser().getId(), getConnection());
...
//    }
    ...
    public String getReportWeekly() throws Exception {

        String selectedDate = getHttpRequestParameter("selectedDate");

        if (StringUtils.isEmpty(selectedDate)) {
            selectedDate = TimeUtils.getNowDate();
        }

        String startTime = TimeUtils.getFirstDayOfWeek(selectedDate);
        String endTime = TimeUtils.getLastDayOfWeek(selectedDate);

        Pager pager = Pager.getInstance(getRequest());
        pager = orderService.getReportWeekly(startTime, endTime, pager.getCurrentPage(), pager.getShowRowCount(), getConnection());
...
//        getResult().setReturnValue(kvObjects.toJSONObject());
    ...
    public String financeMoneyConfirm() throws Exception {
        ...
        orderService.financeMoneyConfirm(orderId, getLoginUser().getId(), getConnection());
    ...
    public String financeMoneyConfirmCancel() throws Exception {

        String orderId = getHttpRequestParameter("orderId");

        orderService.financeMoneyConfirmCancel(orderId, getLoginUser().getId(), getConnection());
    ...
    public String saleAndWaitingOrder() throws Exception {
        ...
        try {
            ...
            StringUtils.checkIsEmpty(order.getPayTime(), "请确认【打款时间】是否正确");
            StringUtils.checkIsEmpty(moneyString, "请确认【操作金额】是否正确");
    ...
    public String appointmentOrder() throws Exception {
        ...
        StringUtils.checkIsEmpty(order.getAppointmentTime(), "预约时间");
        StringUtils.checkIsEmpty(moneyString, "预约金额");

        order.setMoney(Double.parseDouble(moneyString));

        order = orderService.appointmentOrder(order.getId(), order.getCustomerId(), order.getProductionId(), order.getProductionCompositionId(), order.getMoney(), "", true, order.getAppointmentTime(), order.getPayTime(), TimeUtils.getNow(), order.getReferralCode(), order.getStatus(), order.getAccountId(), order.getBankCode(), order.getDescription(), getConnection());
    ...
    public String getListOrderVO() throws Exception {
        ...
        orderVO.setCommonStatus(orderStatus);
        ...
        List<OrderVO> orderVOs = orderService.getListOrderVO(orderVO, null, projectTypeIds, getConnection());
